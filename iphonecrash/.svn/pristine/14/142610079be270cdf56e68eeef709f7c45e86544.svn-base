...
- (void)createBrowseView
{
    ...
    _countLabel.textColor = [UIColor whiteColor];
     CGFloat height =   CGRectGetHeight([UIApplication sharedApplication].statusBarFrame)+20.0f;
    _countLabel.frame = CGRectMake(0, height, _screenWidth, 50);
...
- (void)deviceOrientationDidChange:(NSNotification *)notification
{
    ...
    if(orientation == UIDeviceOrientationPortrait || orientation == UIDeviceOrientationLandscapeLeft || orientation == UIDeviceOrientationLandscapeRight)
    {
        ...
        if(_currentOrientation == UIDeviceOrientationPortrait)
        {
		    ...
		    _countLabel.frame = CGRectMake(0,50, _screenWidth, 50);
        }  else  {
            ...
            if(_currentOrientation == UIDeviceOrientationLandscapeLeft)
            {
			    ...
			    _countLabel.frame = CGRectMake(0, 20, _screenWidth, 50);
            }  else  {
                [UIView animateWithDuration:0.5 animations:^{
                    _bgView.transform = CGAffineTransformMakeRotation(- M_PI / 2);
                }];
