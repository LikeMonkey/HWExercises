...
+(NSString *)token:(NSString *)auth
			  time:(NSString *)time
			   method:(NSString *)method
			   key:(NSString *)key
{
	NSString *aMethod = !NullString(method)?method:@"";
	NSString *aAuth = !NullString(auth)?auth:@"";
	NSString *data_value = [NSString stringWithFormat:@"%@,%@,%@,%@",aAuth,time,aMethod,key];
	NSString *ss = [[[data_value stringByReplacingOccurrencesOfString:@"\"" withString:@"”"]stringByReplacingOccurrencesOfString:@";" withString:@"；" ] stringByReplacingOccurrencesOfString:@"'" withString:@"‘"];
	NSData *data = [ss dataUsingEncoding:NSUTF8StringEncoding];
		//IOS 自带DES加密 Begin  改动了此处
	data = [self DESEncrypt:data WithKey:key];
	NSString *result = [self base64EncodedStringFrom:data];
	return result;
}
@end
