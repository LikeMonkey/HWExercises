...
- (void)setupMainView
{
    ...
    _mainView = mainView;
	if (_isNotShouldPan) {
		_mainView.panGestureRecognizer.enabled = NO;
	}else{
		_mainView.panGestureRecognizer.enabled = YES;
	}
}
-(void)setIsNotShouldPan:(BOOL)isNotShouldPan
{
	_isNotShouldPan  = isNotShouldPan;
	if (_mainView) {
		if (_isNotShouldPan) {
			_mainView.panGestureRecognizer.enabled = NO;
		}else{
