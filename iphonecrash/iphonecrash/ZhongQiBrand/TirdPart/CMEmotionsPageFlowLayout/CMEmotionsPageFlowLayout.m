...
- (void)prepareLayout {
    ...
    for (NSInteger sectionIndex = 0; sectionIndex < sectionCount; sectionIndex++) {
        ...
        for (NSInteger itemIndex = 0 ; itemIndex < itemCount; itemIndex++) {
            ...
            //计算cell的坐标
            CGFloat itemX = self.sectionInset.left + cloIndex * (itemW + self.minimumInteritemSpacing) + (pageIndex + self.pageCount) * self.collectionView.bounds.size.width;
        ...
        self.pageCount += (itemCount - 1) / (self.rowsCount * self.closCount)+1;
    }
}
...
{
    return CGSizeMake(self.pageCount * self.collectionView.bounds.size.width-1, 0);
}
