...
- (void)loadBrowseImageWithBrowseItem:(MSSBrowseModel *)browseItem Cell:(MSSBrowseCollectionViewCell *)cell bigImageRect:(CGRect)bigImageRect
{
    ...
    {
        if (browseItem.bigImage&&browseItem.bigImageUrl==nil) {
             [self showBigImage:cell.zoomScrollView.zoomImageView browseItem:browseItem rect:bigImageRect];
        }else{
            self.isFirstOpen = NO;
            // 加载大图
...
- (void)showBigImage:(UIImageView *)imageView browseItem:(MSSBrowseModel *)browseItem rect:(CGRect)rect
{
    ...
    // 如果存在直接显示图片
    if ([browseItem.bigImage isKindOfClass:NSClassFromString(@"OS_dispatch_data")]) {
        imageView.image = (browseItem.bigImageUrl==nil)?[UIImage sd_animatedGIFWithData:(NSData *)browseItem.bigImage]:[[SDImageCache sharedImageCache]imageFromDiskCacheForKey:browseItem.bigImageUrl];
    }else{
            imageView.image =(browseItem.bigImageUrl==nil)?browseItem.bigImage:[[SDImageCache sharedImageCache]imageFromDiskCacheForKey:browseItem.bigImageUrl];
    }
