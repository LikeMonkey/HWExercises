...
//															FCBaseModel *baseModel = 	[[FCBaseModel alloc]initWithDictionary:responseObject];
//															NSLog(@"当前状态:| %@",baseModel.status);
														}
													}
                                                    if (responseObject)
                                                    {
                                              ...
                                              andIsEcode:NO
                                                compod:^(id responseObject, NSError *aerrror) {
													if (responseObject) {
														if ([responseObject isKindOfClass:[NSDictionary class]]) {
#if DEBUG
															NSString *responseStr = [NSString convertToJsonData:responseObject];
															NSLog(@"%@",responseStr);
#endif
														}
													}
                               ...
                               Error:(void(^)(NSError *errror))anerror
{
    ...
    return [self upload:url andParmrs:mutableDic
			 andIsEcode:YES
...
{
    if (![url isEqualToString:AppendAPI(APPURL_prefix, @"")] && ![url isEqualToString:AppendAPI(APPURL_prefix, @"")]) {
        self.method = method;
        self.urlstr = url;
        self.parms  = requestData;
        self.isEncryption = isEcode;
    }
#ifdef DEBUG
    NSLog(@"\n\n请求参数--------- %@\n\n",requestData);
#endif
...
//                compoletion(result,nil);
                [self mobilePhoneVerification:result error:nil compod:compoletion];
                
            }else  if ([responseObject isKindOfClass:[NSDictionary class]])
            {
//                compoletion(responseObject,nil);
                [self mobilePhoneVerification:responseObject error:nil compod:compoletion];
            }
            else
            {
...
//                compoletion(comp,nil);
                 [self mobilePhoneVerification:comp error:nil compod:compoletion];
            }
...
//                compoletion(result,error);
                [self mobilePhoneVerification:result error:error compod:compoletion];
#if DEBUG
...
-(NSURLSessionDataTask *)upload:(NSString *)url
                      andParmrs:(id)parmrs
                     andIsEcode:(BOOL)isEcode
				andMaxImageSize:(CGFloat)maxImageSize
                  andImagesInfo:(NSDictionary *)imgInfo
                  isCompression:(BOOL)isCompression
                  andCompletion:(void(^)(id obj))completion
                    andProgress:(void(^)(float prog))aProgres
    ...
    return  [self.afmanager POST:url parameters:parmrs constructingBodyWithBlock:^(id<AFMultipartFormData> _Nonnull formData) {
        ...
        for (NSString *key in imgInfo.allKeys) {
            ...
            for (int i = 0; i< images.count; i++)
            {
			   ...
			   if (isCompression) {
				   if (maxImageSize != 0) {
					   imageData = [weakSelf compressImage:image toMaxFileSize:fabs(maxImageSize)];
				   }else{
...
- (void)mobilePhoneVerification:(id)result error:(NSError *)error compod:(void(^)(id responseObject, NSError *aerrror))compoletion{
    
    NSInteger status = [result[@"status"] integerValue];//3 :请验证手机
    if (status == 3) {
        [MobilePhoneVerificationView showWithHandler:^(UIButton * _Nonnull btn) {
            if (btn.tag == 100) {
                NSError *error = [NSError errorWithDomain:@"--请验证手机---3" code:3 userInfo:nil];
                compoletion(nil,error);
            }
            else if (btn.tag == 101){
                [self __request:self.method urlstr:self.urlstr requestData:self.parms andIsEcode:self.isEncryption compod:^(id responseObject, NSError *aerrror) {
                    if (responseObject) {
                        if ([responseObject isKindOfClass:[NSDictionary class]]) {
#if DEBUG
                            NSString *responseStr = [NSString convertToJsonData:responseObject];
